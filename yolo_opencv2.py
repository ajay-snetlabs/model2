...
print(f"Number of objects detected: {len(indices)}")

# Calculate and print confidence statistics
if confidences:
...
with open('detection_log.txt', 'a') as f:
    ...
    f.write(f"[{timestamp}] Detection completed in {detection_time:.4f} seconds. "
            f"Detected {len(indices)} objects. "
            f"Avg confidence: {avg_confidence:.4f} (min: {min_confidence:.4f}, max: {max_confidence:.4f})\n")
