        ...
        for out_layer in outs:
            for detection in out_layer:
                ...
                if confidence > conf_threshold:
                    ...
                    x = int(center_x - w / 2)
                    y = int(center_y - h / 2)
                    class_ids_frame.append(class_id)
                    confidences_frame.append(float(confidence))
                    boxes_frame.append([x, y, w, h])
        indices = cv2.dnn.NMSBoxes(boxes_frame, confidences_frame, conf_threshold, nms_threshold)
        objects_detected = len(indices)
        avg_confidence = sum(confidences_frame) / len(confidences_frame) if confidences_frame else 0

        if indices is not None:
            for i in indices:
                try:
                    box = boxes_frame[i]
                    confidence = confidences_frame[i]
                    class_id = class_ids_frame[i]
                    x, y, w, h = round(box[0]), round(box[1]), round(box[2]), round(box[3])
                    draw_prediction(frame, class_id, confidence, x, y, x + w, y + h)
                except IndexError:
                    continue

        cv2.putText(frame, f"Frame Size: {Width}x{Height}", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        cv2.putText(frame, f"FPS: {1/detection_time:.2f}", (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        cv2.putText(frame, f"Objects: {objects_detected}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        cv2.putText(frame, f"Avg Conf: {avg_confidence*100:.2f}%", (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

        cv2.imshow('Object Detection', frame)
        if out is not None:
            out.write(frame)

        # Print real-time data to the console with a newline
        print(f"Frame Size: {Width}x{Height}, FPS: {1/detection_time:.2f}, Objects Detected: {objects_detected}, Avg Conf: {avg_confidence*100:.2f}%")
